...
## FunTech 社群網站 · 完整試題說明與第一版極致簡化實作文件
...
本版本為第一版極致簡化實作，在**不影響任何畫面與功能**的前提下，對所有程式碼進行最大幅度精簡。

### 簡化成果

| 項目 | 原始行數 | 簡化後行數 | 削減 |
|------|---------|-----------|------|
| 全部 PHP / CSS 檔案 | 898 行 | 580 行 | **−35%** |

### 主要簡化策略

| 策略 | 說明 |
|------|------|
| **SQL 查詢合併** | `login.php` 原本兩次 query（先 count 再 SELECT id），合併為一次 `SELECT id` 即可同時驗證與取 id |
| **PHP 變數提前** | `friends-page.php` 將 `$_SESSION['user_id']` 提前存為 `$my`，避免頁面內重複存取 session |
| **array_filter + fn()** | `search_users.php` 以箭頭函式取代 foreach unset，`pull_score.php` 以 `array_map` 取代 foreach 建陣列 |
| **switch → default 合併** | `set_friend.php` 的 cancel / reject / remove 三個 case 邏輯相同（DELETE），合併為 `default` |
| **`$msgs` 陣列** | `set_friend.php` 回應訊息改用關聯陣列查表，取代五個 case 各自 echo |
| **移除死碼與冗餘** | `register.php` 移除已註解的 password 重複檢查；`article.php` 移除被完整註解的「返回」區塊；`index.js` 內容直接寫入 `index.php` `<head>` 消除一個 HTTP 請求 |
| **`@unlink` 靜默錯誤** | `update_avatar.php` 改用 `@unlink` 取代 strlen 判斷，縮短一行 |
| **CSS / HTML 空白** | 壓縮多餘縮排與空行（不影響可讀性的範圍內） |

### 同步修正的 Bug

| 檔案 | Bug | 修正 |
|------|-----|------|
| `front/friend-profile-page.php` | `status=='pendding'`（typo） | 改為 `status=='pending'` |
| `front/profile-page.php` | `<textarea id="bio">` selector 需與 class 一致 | 改為 `class="profile-bio-input"` 以符合 CSS Selector 規格 |
| `front/profile-page.php` | `<span class="show-bio">` 未帶 `profile-bio` class | 補上 `profile-bio` 以符合 3.2.2.3 selector |
| `front/friends-page.php` | 搜尋輸入用 `id="search"` 但 JS 用 class selector | 統一改為 class `.search-input` |
| `front/add-article.php` | JS 直接用 `id` 而非 class，不符合競賽 selector 規格 | 改為 class selector |
| `api/register.php` | 重複帳號時 echo script 跳轉語法錯誤 | 改為 echo 0，由前端統一顯示 alert |
| `index.php` | navbar 缺少 `.user-badge`、`.login-link`、`.register-link` 等 class | 補齊所有規格 class |
...
| 1.2.2.3.1 | 未登入：`a.login-link` + `a.register-link` | `.user-area a.login-link` / `.user-area a.register-link` | ✅ |
...
| 3.2.2.1 | 使用者頭像 | `.profile-header img.profile-avatar` | ✅ |
...
| 3.1 | 好友列表標題 | `.friend-list-section .section-title` | ✅ |
...
| 2.4.3 | 排行榜資料項目 | `.game-leaderboard .leaderboard-item` | ✅ 動態呼叫 `score.pullUrl` |
| 2.4.4 | 顯示名次 + API 所有欄位 | `.leaderboard-item .player-rank` | ✅ |
| 2.4.5 | 空資料顯示「目前尚無分數紀錄」 | — | ✅ |
...
```
th56j_national/
├── index.php              # 主入口（含 Header 導覽列、loadpage 函式）
├── front/
...
├── games/
│   └── 1/ ~ 5/            # 5 個遊戲資料夾（含 game.json、api/pull_score.php）
├── assets/
│   ├── css/index.css      # 自訂 CSS（Bootstrap CDN 引入）
